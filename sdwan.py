...
def sdwanlogin(session,inputdata):
    ...
    # Authentication to Cisco SDWAN (Viptela) is against the vManage orchestrator with a series of steps
    # 1) pass the authentication credentials in the body and retrive JSESSIONID Cookie
    #    credentials can be passed as a dictionary of two elements or a single element (commented below)
    # 2) Request a Cross site forgery request token (XSRF Token) that is required for POST operations with the JSESSIONID Cookie
    #    reusing the same session (carrying JESSIONID cookie) we can request an XSRF token that will be returned in the BODY
    #    of the response (if successful)
    # 3) Once Cookie is retained and the XSRF token is obtained, is possible to make requests to the vManage via REST API for both
...
def sdwantoken(session,inputdata):
    ...
    # Step 2 --- Retrieve XSRF token and update the session headers to include it in future requests (cookie JESSIONID is retained)
